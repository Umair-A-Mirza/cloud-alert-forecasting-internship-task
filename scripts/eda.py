...
from utils import _validate_df, _validate_anomaly_windows


def basic_summary(df: pd.DataFrame, should_print: bool = False) -> Dict:
    """Prints basic information for the series, including min, max, sampling interval,
    missing intervals (based on median), and basic summary statistics."""
...
def plot_distribution(
    df: pd.DataFrame, bins: int = 40, title: str = "Distribution"
) -> None:
...
def plot_rolling_statistics(
    df: pd.DataFrame,
    window: int = 12,
    sampling_interval: pd.Timedelta = pd.Timedelta(minutes=5),
    title: str = "Rolling Statistics",
) -> None:
    """Plots the rolling mean and standard deviation.

    Note that if the dataset is sampled every five minutes, adjustments should be made
    to provide hourly or daily analysis.
    """
    ...
    axes[0].plot(
        ...
        rolling_mean,
        linewidth=2,
        label=f"Rolling Mean ({duration_str})",
    )
    ...
    axes[1].plot(
        ...
        rolling_std,
        linewidth=1.5,
        label=f"Rolling Std ({duration_str})",
    )
...
def plot_dayofweek_mean(df: pd.DataFrame, title: str = "Day-of-Week Pattern") -> None:
    ...
    df["day_name"] = pd.Categorical(
        df["timestamp"].dt.day_name(), categories=day_order, ordered=True
    )
...
def plot_boxplot_by_hour(
    df: pd.DataFrame, title: str = "Boxplot by Hour of Day"
) -> None:
    _validate_df(df)
...
def plot_boxplot_by_dayofweek(
    df: pd.DataFrame, title: str = "Boxplot by Day of Week"
) -> None:
    _validate_df(df)
...
def adf_test(
    df: pd.DataFrame, value_col: str = "value", should_print: bool = False
) -> Dict:
    """
    ...
    if should_print:
        print(
            tabulate(summary.items(), headers=["Property", "Value"], tablefmt="simple")
        )
...
def compute_anomaly_statistics(
    ...
    """Given that Exploratory Data Analysis (in this case) contains information about
    anomalous windows, we can extract some data from this.

    In any domain, we should know how frequent anomalous behavior is in order to assess
    how.
    """
    ...
    avg_duration = (
        total_duration / len(anomaly_windows) if anomaly_windows else pd.Timedelta(0)
    )
...
def plot_lag(
    ...
    """Lag Plots are a useful way to understand some properties of a time series,
    particularly seasonality when a repeating structure is seen at a particular lag.

    This lag plot also includes different colors for different cases (neither anomalous,
    anomalous at t, anomalous at t-lag, both anomalous).
    """
    ...
    if should_plot:
        ...
        if dominant_freq_per_sample > 0:
            ax.axvline(
                ...
                alpha=0.8,
                label=(
                    f"Dominant Frequency = {dominant_freq_per_sample} cycles/sample"
                ),
            )
            ax.legend()
